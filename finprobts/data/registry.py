...
def _crsp_loader(**kwargs) -> FinancialDataset:
    """Load a local CRSP-derived panel without committing licensed data.

    The loader intentionally delegates to the canonical wide/long table loader.
    A project-specific config supplies the local path, date column, asset id
    column, and return/price target fields.
    """

    kwargs = dict(kwargs)
    kwargs.setdefault("format", kwargs.pop("data_format", "wide"))
    metadata = dict(kwargs.pop("metadata", {}) or {})
    metadata.setdefault("dataset_name", "crsp")
    metadata.setdefault("license", "local_private")
    return load_financial_data(metadata=metadata, **kwargs)
...
def get_default_dataset_registry() -> DatasetRegistry:
    ...
    registry.register("crsp", _crsp_loader)
