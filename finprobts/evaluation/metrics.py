...
The public benchmark metrics prefer established scoring packages when they are
available. Local implementations are retained as deterministic fallbacks and
for finance diagnostics that are not provided by the scoring libraries.
"""
...
def metric_backend_versions() -> Dict[str, Optional[str]]:
    """Return metric backend package versions captured in run metadata."""

    return {
        "scoringrules": _package_version("scoringrules"),
        "properscoring": _package_version("properscoring"),
        "gluonts": _package_version("gluonts"),
    }
...
def crps_sample(samples: np.ndarray, y_true: np.ndarray) -> float:
    ...
    Preference order:
    1. ``scoringrules.crps_ensemble`` when installed.
    2. ``properscoring.crps_ensemble`` when installed.
    3. Local empirical CRPS fallback.
    ...
    observations = y_true.reshape(-1)
    forecasts = np.moveaxis(samples, 1, -1).reshape(-1, sample_count)

    try:
        import scoringrules as sr  # type: ignore

        return float(np.mean(sr.crps_ensemble(observations, forecasts)))
    except Exception:
        pass
    ...
    try:
        ...
        return float(np.mean(ps.crps_ensemble(observations, forecasts)))
    except Exception:
        pass
...
def energy_score_sample(samples: np.ndarray, y_true: np.ndarray) -> Optional[float]:
    """Multivariate energy score from forecast samples.

    This is useful for dependence-sensitive evaluation. It is computed locally
    because package APIs are less stable across probabilistic-scoring stacks.
    """
    ...
    if sample_count < 2:
        return None
    sample_vectors = samples.reshape(samples.shape[0], sample_count, -1)
    truth_vectors = y_true.reshape(y_true.shape[0], -1)
    term1 = np.linalg.norm(sample_vectors - truth_vectors[:, np.newaxis, :], axis=-1).mean(axis=1)
    # O(S^2), acceptable for benchmark sample counts used in reporting. Large
    # leaderboard runs can disable this metric in config if needed.
    pairwise = sample_vectors[:, :, np.newaxis, :] - sample_vectors[:, np.newaxis, :, :]
    term2 = 0.5 * np.linalg.norm(pairwise, axis=-1).mean(axis=(1, 2))
...
def evaluate_forecasts(
    ...
    include_multivariate_scores: bool = False,
) -> Dict[str, Any]:
    ...
    if include_multivariate_scores:
        metrics["energy_score"] = energy_score_sample(samples, y_true)
