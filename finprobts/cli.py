...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    if not config_path:
        config_path = _write_tslib_style_config(args)
...
def _write_tslib_style_config(args: argparse.Namespace) -> str:
    """Translate Time-Series-Library-style flags into a resolved YAML config."""

    if not args.model:
        raise SystemExit("run requires --config or --model.")
    data_name = str(args.data or "custom_csv").lower()
    dataset_name = "crsp" if data_name == "crsp" else data_name
    if dataset_name in {"custom", "csv"}:
        dataset_name = "custom_csv"
    if dataset_name in {"parquet", "pq"}:
        dataset_name = "custom_parquet"

    data_path = args.data_path
    if args.root_path and args.data_path:
        data_path = str(Path(args.root_path) / args.data_path)
    if not data_path:
        raise SystemExit("flag-style run requires --data_path, optionally with --root_path.")
    ...
    config = {
        "run": {
            "run_id": args.run_id or f"{dataset_name}_{str(args.model).lower()}",
        ...
        "dataset": {
            ...
            "target_columns": args.target_columns.split(",") if args.target_columns else None,
            "asset_id_column": args.asset_id_column,
            "target_column": args.target_column,
            "freq": args.freq,
            "validate_regular": args.validate_regular,
        },
        "preprocessing": {
            "value_kind": args.value_kind,
            "price_to_log_return": args.price_to_log_return,
        ...
        "forecast": {"num_samples": args.num_samples},
        "evaluation": {
            "quantiles": [0.1, 0.5, 0.9],
            "coverage_levels": [0.5, 0.9],
            "var_alpha": args.var_alpha,
            "include_multivariate_scores": args.include_multivariate_scores,
        },
    }
    config_dir = Path("configs/generated/flag_runs")
    config_dir.mkdir(parents=True, exist_ok=True)
    config_path = config_dir / f"{config['run']['run_id']}.yaml"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--task_name", default="probabilistic_forecast")
    run.add_argument("--is_training", type=int, default=1)
    run.add_argument("--model", help="Model name, e.g. DeepVAR or quantileformer.")
    run.add_argument("--data", default="custom_csv", help="Dataset registry name, e.g. crsp.")
    run.add_argument("--root_path", help="Optional root directory for --data_path.")
    run.add_argument("--data_path", help="CSV/Parquet data path.")
    run.add_argument("--data_format", default="wide", choices=["wide", "long"])
    run.add_argument("--date_column", default="date")
    run.add_argument("--target_columns", help="Comma-separated wide target columns.")
    run.add_argument("--asset_id_column", default="asset_id")
    run.add_argument("--target_column", default="target")
    run.add_argument("--freq", default=None)
    run.add_argument("--validate_regular", action=argparse.BooleanOptionalAction, default=True)
    run.add_argument("--value_kind", default="returns", choices=["returns", "prices"])
    run.add_argument("--price_to_log_return", action="store_true")
    run.add_argument("--missing_method", default="ffill")
    run.add_argument("--standardize", action=argparse.BooleanOptionalAction, default=True)
    run.add_argument("--train_size", type=float, default=0.6)
    run.add_argument("--val_size", type=float, default=0.2)
    run.add_argument("--seq_len", type=int, default=96)
    run.add_argument("--pred_len", type=int, default=1)
    run.add_argument("--stride", type=int, default=1)
    run.add_argument("--num_samples", type=int, default=100)
    run.add_argument("--train_epochs", type=int, default=10)
    run.add_argument("--batch_size", type=int, default=64)
    run.add_argument("--learning_rate", type=float, default=1.0e-3)
    run.add_argument("--device", default="auto")
    run.add_argument("--seed", type=int, default=2026)
    run.add_argument("--run_id")
    run.add_argument("--output_dir", default="outputs")
    run.add_argument("--var_alpha", type=float, default=0.05)
    run.add_argument("--include_multivariate_scores", action="store_true")
